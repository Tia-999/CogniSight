...
st.markdown(
    """
    <style>
    body, .main, .block-container {background-color: #00B4D8; color: blue;}
...
if st.button('🚀 Run Detection'):
    with st.spinner("Analyzing text with model..."):
        ...
        if use_adaptive:
            ...
            score = min_k_percent_prob(lps, k_percent=0.1)
...
if st.button('📡 Run AUC Evaluation'):
    with st.spinner('Loading dataset and running detection...'):
        ...
        for i, txt in enumerate(texts):
            ...
            score, _ = adaptive_min_prob(lps, method=method) if use_adaptive else (min_k_percent_prob(lps, k_percent=0.3), None)
            scores.append(score)
